...
- (void)viewWillAppear:(BOOL)animated {
    ...
    // 获取权限状态
    [manager getEnabledStatusForExtensionWithIdentifier:ExtensionIdentifier completionHandler:^(CXCallDirectoryEnabledStatus enabledStatus, NSError * _Nullable error) {
        if (!error) {
            NSString *title = nil;
...
//    self.dataSource = [[CallManager shareManager] readFile];
    ...
    __block NSString *sql = [NSString stringWithFormat:@"select number, name from backPhoneNumber order by number asc"];
    __block NSMutableArray *tempArray = [[NSMutableArray alloc] init];
    tempArray =  [[Fmdbmode share] executeQuery:sql];
    
    
    for (NSDictionary *phone in tempArray) {
        @autoreleasepool {
            if (phone[@"number"]) {
