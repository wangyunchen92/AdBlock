...
+ (BOOL)readFileAndDel {
    ...
    containerURL = [containerURL URLByAppendingPathComponent:@"MyDatabase.db"];
    NSString * newfilePath = containerURL.path;
    NSBundle *bundle = [NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:@"backSql" ofType:@"bundle"]];
    NSString *filePath = [bundle pathForResource:@"personBack1" ofType:@"db"];
    NSError *error;
    NSData *date = [NSData dataWithContentsOfFile:filePath options:NSDataReadingMappedIfSafe error:&error];
    if (date.length == 0 || !date) {
        return NO;
    }
    if (!error) {
        
        if ([date writeToFile:newfilePath atomically:YES]) {
